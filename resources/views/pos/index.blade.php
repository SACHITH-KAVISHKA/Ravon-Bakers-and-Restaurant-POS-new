...
<body>
        ...
        <div class="pos-content">
            ...
            <div class="items-panel">
                ...
                <div class="items-grid" id="items-container">
                    ...
                    $pickmeBranchPrice = $item->branchPrices->firstWhere('branch_id', 8)?->price ?? 0;
                    @endphp
                    <div class="item-card"
                        ...
                        data-pickme-price="{{ $pickmeBranchPrice }}"
                        ...
                        @if($pickmeBranchPrice > 0)
                            <small class="text-muted d-block" style="font-size: 10px;">PickMe: LKR {{ number_format($pickmeBranchPrice, 2) }}</small>
                        @endif
                    </div>
                    @endforeach
                </div>
            ...
            <div class="payment-panel d-none d-md-flex">
                ...
                <!-- Order Type Selection for Pricing -->
                <div class="p-2 bg-light border-bottom">
                    <label class="form-label small fw-bold text-secondary mb-1">ORDER TYPE (PRICING)</label>
                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-sm btn-outline-primary flex-fill active" id="order-type-local" onclick="changePriceTier('local')">
                            <i class="bi bi-shop"></i> Normal Price
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-warning flex-fill" id="order-type-pickme" onclick="changePriceTier('pickme')">
                            <i class="bi bi-bicycle"></i> PickMe Price
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        // PickMe Branch ID එක (Database එකේ වැටෙන ID එක මෙතනට දාන්න)
        const PICKME_BRANCH_ID = 8;
        let currentPriceTier = 'local'; // default configuration

        function changePriceTier(tier) {
            currentPriceTier = tier;
            document.getElementById('order-type-local').classList.toggle('active', tier === 'local');
            document.getElementById('order-type-pickme').classList.toggle('active', tier === 'pickme');


            const removedItems = [];

            cart = cart.filter(cartItem => {
                const cardElement = document.querySelector(`.item-card[data-item-id="${cartItem.id}"]`);
                if (!cardElement) return true;

                if (tier === 'pickme') {
                    const pickmePrice = parseFloat(cardElement.dataset.pickmePrice);
                    if (!cardElement.dataset.pickmePrice || isNaN(pickmePrice) || pickmePrice <= 0) {
                        removedItems.push(cartItem.name);
                        return false;
                    }
                    cartItem.price = pickmePrice;
                } else {
                    cartItem.price = parseFloat(cardElement.dataset.itemPrice);
                }
                return true;
            });

            if (removedItems.length > 0) {
                showError('No PickMe price - removed: ' + removedItems.join(', '));
            }
        ...
        function addToCartFromCard(card) {
            ...
            let price = parseFloat(card.dataset.itemPrice);
            ...
            if (currentPriceTier === 'pickme') {
                if (card.dataset.pickmePrice && parseFloat(card.dataset.pickmePrice) > 0) {
                    price = parseFloat(card.dataset.pickmePrice);
                } else {
                    showError(itemName + ' - PickMe price not set for this item');
                    return;
                }
            }
        ...
        function processModalPayment() {
            ...
            const orderData = {
                ...
                is_pickme: (currentPriceTier === 'pickme')
            };

        const printBtn = document.getElementById('modal-print-btn');
            if (printBtn.disabled && printBtn.innerHTML.includes('Processing')) return;
                ...
                .then(async (data) => {
                    if (data.success || data.receipt_no) {
                        ...
                        try {
                            ...
                            showError("Payment was SUCCESSFUL, but printing failed.");
                        }
                        ...
                        showError(data.error || data.message || 'Error processing payment');
                    }
                })
                .catch(error => {
